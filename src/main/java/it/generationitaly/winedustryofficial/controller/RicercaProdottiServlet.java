...
public class RicercaProdottiServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (!prodottoAnnata.isEmpty()) {
			List<Prodotto> prodottiByAnnata = new ProdottoJPA(Prodotto.class).findBySomethingList("where annata = ?1",
					prodottoAnnata);
		...
		if (!req.getParameter("prodottoFormato").isEmpty()) {
			List<Prodotto> prodottiByFormato = new ProdottoJPA(Prodotto.class).findBySomethingList("where formato = ?1",
					req.getParameter("prodottoFormato"));
		...
		if (!req.getParameter("prodottoGradazione").isEmpty()) {
			List<Prodotto> prodottiByGradazione = new ProdottoJPA(Prodotto.class)
					.findBySomethingList("where gradazione = ?1", req.getParameter("prodottoGradazione"));
		...
		if (!req.getParameter("prodottoCategoria").isEmpty()) {
			List<Prodotto> prodottiByCategoria = new ProdottoJPA(Prodotto.class)
					.findBySomethingList("where categoria = ?1", req.getParameter("prodottoCategoria"));
		...
		if (!req.getParameter("ricercaProdotto").isEmpty()) {
			...
			if (isNumeric(value)) {
				List<Prodotto> prodottiByAnnata = new ProdottoJPA(Prodotto.class)
						.findBySomethingList("where annata = ?1", value);
				if (!prodottiByAnnata.isEmpty()) {
				...
				List<Prodotto> prodottiByFormato = new ProdottoJPA(Prodotto.class)
						.findBySomethingList("where formato = ?1", value);
				if (!prodottiByFormato.isEmpty()) {
